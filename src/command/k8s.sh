...
alias k8s-pods-create="kubectl pods create --kubeconfig ~/.kube/config -n ${NAMESPACE_DEFAULT}"
alias k8s-pods-delete="kubectl pods delete --kubeconfig ~/.kube/config -n ${NAMESPACE_DEFAULT}"
...
alias k8s-create-cron-force-sync="kubectl create job --from=cronjob/${NAMESPACE_DEFAULT}-cron-force-sync-image -n ${NAMESPACE_DEFAULT} ${NAMESPACE_DEFAULT}-cron-force-sync-image-$(date | sha1sum | tr -d ' -')"
